...
namespace GPU
{
	...
	void VK_GraphicsPipeline::CreatePipeline()
	{
		...
		const VK_RenderPassDrawInfo pCurrenDrawInfo = VK_Backend::Get()->GetCurrentDrawInfo();
		...
		/* 
			# Check if any thing is changed from the last use of this pipeline..
		      if every thing is the same don't reCreate the pipeline just use it again.
		*/
		if (pUsedDrawInfo.pColorFormats.size() == pCurrenDrawInfo.pColorFormats.size())
		{
			for (uint32_t i = 0; i < pUsedDrawInfo.pColorFormats.size(); i++)
			{
				if (pUsedDrawInfo.pColorFormats[i] != pCurrenDrawInfo.pColorFormats[i])
				{
					IsColorAttachmentTheSame = false;
					break;
				}
			}
		}
		...
		if (
			!IsColorAttachmentTheSame ||
			pUsedDrawInfo.pDepthFormat != pCurrenDrawInfo.pDepthFormat ||
			pUsedDrawInfo.pEnableColor != pCurrenDrawInfo.pEnableColor ||
			pUsedDrawInfo.pEnableDepth != pCurrenDrawInfo.pEnableDepth
			)
		{
			pUsedDrawInfo.pEnableColor = pCurrenDrawInfo.pEnableColor;
			pUsedDrawInfo.pEnableDepth = pCurrenDrawInfo.pEnableDepth;
			pUsedDrawInfo.pColorFormats = pCurrenDrawInfo.pColorFormats;
			pUsedDrawInfo.pDepthFormat = pCurrenDrawInfo.pDepthFormat;
			pUsedDrawInfo.pWidth = pCurrenDrawInfo.pWidth;
			pUsedDrawInfo.pHeight = pCurrenDrawInfo.pHeight;
			const bool IsColorAttc = pUsedDrawInfo.pEnableColor;
			const bool IsDepthTest = pUsedDrawInfo.pEnableDepth;
			...
			VkViewport VP = {
				.x = 0.0f,
				.y = 0.0f,
				.width = (float)pUsedDrawInfo.pWidth,
				.height = (float)pUsedDrawInfo.pHeight,
				.minDepth = 0.0f,
				.maxDepth = 1.0f
			};
			...
			VkRect2D Scissor = {
				...
				.extent = {
					.width = pUsedDrawInfo.pWidth, .height = pUsedDrawInfo.pHeight
				}
			};
			...
			VkPipelineRenderingCreateInfo RenderingInfo = {
				...
				.pNext = NULL,
				.colorAttachmentCount = (uint32_t)pUsedDrawInfo.pColorFormats.size(),
				.pColorAttachmentFormats = pUsedDrawInfo.pColorFormats.data(),
				.depthAttachmentFormat = IsDepthTest ? pUsedDrawInfo.pDepthFormat : VK_FORMAT_UNDEFINED,
